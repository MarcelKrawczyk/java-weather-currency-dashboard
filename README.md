...
- Implemented: one-shot fetch of weather, Fixer rate and NBP rate; Wikipedia panel; background execution with a status label.
- Despite the name, this is not yet a dashboard: there is no refresh, no history and no time series, and each Fetch is independent.
- Build: `Main` imports `javafx.*`, which is not part of the JDK since Java 11, and the `pom.xml` has no dependencies. `mvn compile` will fail unless the JDK bundles JavaFX (for example Liberica Full or Zulu FX). There is also no exec or JavaFX plugin configured.
- The Maven coordinates are still IDE defaults (`org.example:TPO1.1`).
- `Service.getHTML()` and the `country` field are unused.

## Roadmap

1. Make the build self-contained: add `org.openjfx:javafx-web` and `org.openjfx:javafx-swing` (version matching Java 21) and the JavaFX Maven plugin; set proper coordinates.
2. Read API keys from environment variables and never include the request URL in messages shown to the user.
3. Extract the parsing and the cross-rate calculation into pure, tested functions with fixture JSON (including error responses, missing keys and a zero base rate).
4. Replace the string scanning with a JSON parser, and `HttpURLConnection` with `java.net.http.HttpClient` (which also makes async calls and HTTPS defaults simple).
5. Run the three requests concurrently; add a short-lived cache and a retry with backoff.
6. Keep the as-of time with every rate and show it. Use HTTPS for NBP. Show the difference between the Fixer and NBP rates in basis points.
7. Use `BigDecimal` with explicit rounding for anything that becomes an amount, format with an explicit `Locale`, and choose display precision from the size of the rate.
8. Build the country lookup once (static map), and treat an unknown country or an invalid currency code as an input error instead of defaulting to USD.
